...
Use repo files only when the script is a shared team-maintained tool. If it is private Codex automation behavior, keep it under the automation id. This applies to automation creation generally, not to one specific workflow.
...
5. Put deterministic work in `scripts/` and keep tests beside it. Every new cron/workspace automation should get at least one helper entrypoint, even if the first version only validates inputs and emits structured metadata.
6. Put structured non-secret inputs in `data/`, reusable output bodies in `templates/`, and runbooks/design notes in `docs/`.
7. Update the automation prompt to call helper scripts by absolute path, then reason over their structured output.
...
- Run `/Users/seongho/.codex/automations/<automation-id>/scripts/<script>` first.
...
- Run the helper's focused tests, for example `node --test /Users/seongho/.codex/automations/<id>/scripts/*.test.mjs`.
